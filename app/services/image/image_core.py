...
class ImageGenerator:
    ...
    def __init__(self, progress_callback: Optional[Callable[[float], None]] = None):
        ...
        logger.info(f"ImageGenerator initialized: device={self.memory_manager.cuda_device}")
    ...
    def unpack(
        self, dec: torch.Tensor, width: int, height: int, parallel_size: int = 2
    ) -> np.ndarray:
        """
        디코딩된 텐서를 이미지 배열로 변환
    ...
    def generate(
        ...
        try:
            ...
            if token_times:
                ...
                logger.debug(f"Average time per token: {avg_time_per_token*1000:.2f} ms")
            ...
            logger.info(f"Image generation complete, time taken: {generation_time:.2f} seconds")
